...
def Hamiltonian(argument, p, q, kx, ky):
    ...
    for m in range(0, q):
        ...
        h11[m][m] = (t11 + 3 * t22) * cos(2 * pi * alpha * (m + 1) - ky * a)
        h22[m][m] = (3 * t11 + t22) * cos(2 * pi * alpha * (m + 1) - ky * a)
        h12[m][m] = sqrt(3) * (t11 - t22 - 4 * t12) * cos(2 * pi * alpha * (m + 1) - ky * a)

        if m == 0:  #### First row

            h0[m][m + 1] = t0 * (1 + exp(2 * pi * m * alpha - ky * a))  #### Thành phần ma trận thứ hai của dòng đầu tiên
            h0[m][q - 1] = t0 * (1 + exp(2 * pi * m * alpha + ky * a)) * (np.exp(-1j * kx * q * a))  # Thành phân ma trận cuối cùng của dòng đầu tiên
            ...
            h0[m][m - 1] = t0 * (1 + exp(2 * pi * m * alpha + ky * a))
            h0[q - 1][0] = t0 * (1 + exp(2 * pi * m * alpha - ky * a))
