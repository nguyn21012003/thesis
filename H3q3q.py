...
def Hamiltonian(argument, p, q, kx, ky):
    ...
    for m in range(0, q):
        ...
        if m == 0:  #### First row
            ...
            h0[m][m + 1] = t0 * cos(2 * pi * alpha * (m + 1) - ky * a)  #### Thành phần ma trận đầu tiên của dòng đầu tiên
            h0[m][q - 1] = t0 * (np.exp(-1j * q * a))  # Thành phân ma trận cuối cùng của dòng đầu tiên

            h1[m][m + 1] = (t1 + sqrt(3) * t2) / 2 * cos(2 * pi * alpha * (m + 1) - ky * a)
            h1[m][q - 1] = -(t1 + sqrt(3) * t2) / 2 * np.exp(-1j * q * a)
            ...
            for q in range(1, qmax + 1):
                if q > p:
                    if gcd(p, q) == 1:
                        ...
                        # print(H, "\n")
                        eigenvalue2 = np.linalg.eigvalsh(Hamiltonian(choice, p, q, kx=0 * 4 * pi / (3 * a * q), ky=0))
                        # eigenvalue1 = np.linalg.eigvalsh(
                        #     Hamiltonian(choice, p, q, kx=0, ky=0)
                        # )
                        writer.writerow(
                            {
                                f"{'p':^10}": f"{p:^10}",
                        ...
                        plt.plot(y, eigenvalue2, "o", c="red", markersize=0.5)
