...
def Hamiltonian(argument, p, q, kx, ky):
    ...
    for m in range(0, q):
        ...
        h0[m][m] = (
            E_R0[0][0]
            + E_R2[0][0] * exp(1j * (2 * pi * m * alpha - ky * alattice))
            + E_R3[0][0] * exp(1j * (2 * pi * m * alpha - ky * alattice))
            + E_R5[0][0] * exp(-1j * (2 * pi * m * alpha - ky * alattice))
            + E_R6[0][0] * exp(-1j * (2 * pi * m * alpha - ky * alattice))
        )
...
def main():
    ...
    plt.title(f"{matt}")
    # file = f"3band_dataHofstadterButterfly_harper_q_{qmax}_{matt}_test.dat"
    file = f"1band_dataHofstadterButterfly_harper_q_{qmax}_h0.dat"
    # file = f"1band_dataHofstadterButterfly_harper_q_{qmax}_h11.dat"
    # file = f"1band_dataHofstadterButterfly_harper_q_{qmax}_h22.dat"

    kpoints = {
        "G": [0, 0],
        "K": [4 * pi / (3 * alattice * qmax), 0],
        "M": [pi / (alattice * qmax), pi / (sqrt(3) * alattice * qmax)],
    }
    ...
    with open(file, "w", newline="") as writefile:
        ...
        for p in tqdm(range(1, qmax + 1)):
            if gcd(p, qmax) == 1:
                ...
                eigenvalue1 = LA.eigvalsh(Hamiltonian(choice, p, qmax, kx=kpoints["G"][0], ky=kpoints["G"][1]))
    ...
    filegnu = f"plotHofstadterButterfly_harper_q={qmax}_{matt}.gnuplot"
...
set title 'HofstadterButterfly {matt}'
set xlabel 'Alpha'
set ylabel 'Energy'
set grid
#set xrange [0:0.12]
#set yrange [1.3:2.0]
plot '{file}' u 1:2 with points pt 7 ps 0.3 lc rgb 'black' title 'HofstadterButterfly {matt}'
pause -1
        """
        )
