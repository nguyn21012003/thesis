...
def build_tridiagonal_matrix(H_list, t1, t2, q):
    ...
    for i in range(q):
        ...
        if i < q - 1:
            H_qxq[3 * i : 3 * (i + 1), 3 * (i + 1) : 3 * (i + 2)] = t1  # Upper diagonal (t1 as identity matrix)
            H_qxq[3 * (i + 1) : 3 * (i + 2), 3 * i : 3 * (i + 1)] = t1  # Lower diagonal (t1 as identity matrix)

        # Second upper and lower diagonals with t2
        if i < q - 2:
            H_qxq[3 * i : 3 * (i + 1), 3 * (i + 2) : 3 * (i + 3)] = t2  # Upper diagonal (t2 as identity matrix)
            H_qxq[3 * (i + 2) : 3 * (i + 3), 3 * i : 3 * (i + 1)] = t2  # Lower diagonal (t2 as identity matrix)
