...
def eigenvalue(argument):
    ...
    for i in range(N):
        for j in range(N):
            ...
            H3x3 = np.array([[h0, h1, h2], [h1dag, h11, h12], [h2dag, h12dag, h22]])
            H0 = np.kron(I2, H3x3)
    ...
    # plt.subplot(1, 3, 3)
    plt.plot(kx, L1, color="#FF0000")
    plt.plot(kx, L2, color="#FF8000")
    plt.plot(kx, L3, color="#bfff00")
    plt.plot(kx, L4, color="#00FFBF")
    plt.plot(kx, L5, color="#FF00BF")
    plt.plot(kx, L6, color="#00ff0b")
    ...
    with open("test2.txt", "w", newline="") as writefile:
