...
def Hamiltonian(argument, p, q, kx, ky):
    ...
    for m in range(0, q):
        ...
        if m == 0:  #### First row
            ...
            h0[m][m + 1] = E_R1[0][0] * exp(1j * kx * a)  #### Thành phần ma trận thứ hai của dòng đầu tiên
            h1[m][m + 1] = E_R1[0][1] * exp(1j * kx * a)
            h2[m][m + 1] = E_R1[0][2] * exp(1j * kx * a)
            h1T[m][m + 1] = E_R1[1][0] * exp(1j * kx * a)
            h11[m][m + 1] = E_R1[1][1] * exp(1j * kx * a)
            h12[m][m + 1] = E_R1[1][2] * exp(1j * kx * a)
            h2T[m][m + 1] = E_R1[2][0] * exp(1j * kx * a)
            h12T[m][m + 1] = E_R1[2][1] * exp(1j * kx * a)
            h22[m][m + 1] = E_R1[2][2] * exp(1j * kx * a)

        elif m == q - 1:  ### last Row

            h0[m][m - 1] = E_R4[0][0] * exp(-1j * kx * a)
            h1[m][m - 1] = E_R4[0][1] * exp(-1j * kx * a)
            h2[m][m - 1] = E_R4[0][2] * exp(-1j * kx * a)
            h1T[m][m - 1] = E_R4[1][0] * exp(-1j * kx * a)
            h11[m][m - 1] = E_R4[1][1] * exp(-1j * kx * a)
            h12[m][m - 1] = E_R4[1][2] * exp(-1j * kx * a)
            h2T[m][m - 1] = E_R4[2][0] * exp(-1j * kx * a)
            h12T[m][m - 1] = E_R4[2][1] * exp(-1j * kx * a)
            h22[m][m - 1] = E_R4[2][2] * exp(-1j * kx * a)

        else:

            h0[m][m + 1] = E_R1[0][0] * exp(1j * kx * a)  ## thành phần bên trái đường chéo
            h0[m][m - 1] = E_R4[0][0] * exp(-1j * kx * a)  ## thành phần bên phải đường chéo
        ...
        if q == 2:
            ...
            h0[0, q - 1] = E_R1[0][0] + E_R1[0][0] * np.exp(-1j * q * kx * alattice)
            h0[q - 1, 0] = E_R4[0][0] + E_R4[0][0] * np.exp(1j * q * kx * alattice)

            h1[0, q - 1] = E_R1[0][1] + E_R1[0][1] * np.exp(-1j * q * kx * alattice)
            h1[q - 1, 0] = E_R4[0][1] + E_R4[0][1] * np.exp(1j * q * kx * alattice)

            h2[0, q - 1] = E_R1[0][2] + E_R1[0][2] * np.exp(-1j * q * kx * alattice)
            h2[q - 1, 0] = E_R4[0][2] + E_R4[0][2] * np.exp(1j * q * kx * alattice)

            h1T[0, q - 1] = E_R1[1][0] + E_R1[1][0] * np.exp(1j * q * kx * alattice)
            h1T[q - 1, 0] = E_R4[1][0] + E_R4[1][0] * np.exp(-1j * q * kx * alattice)

            h11[0, q - 1] = E_R1[1][1] + E_R1[1][1] * np.exp(-1j * q * kx * alattice)
            h11[q - 1, 0] = E_R4[1][1] + E_R4[1][1] * np.exp(1j * q * kx * alattice)

            h12[0, q - 1] = E_R1[1][2] + E_R1[1][2] * np.exp(-1j * q * kx * alattice)
            h12[q - 1, 0] = E_R4[1][2] + E_R4[1][2] * np.exp(1j * q * kx * alattice)

            h2T[0, q - 1] = E_R1[2][0] + E_R1[2][0] * np.exp(1j * q * kx * alattice)
            h2T[q - 1, 0] = E_R4[2][0] + E_R4[2][0] * np.exp(-1j * q * kx * alattice)

            h12T[0, q - 1] = E_R1[2][1] + E_R1[2][1] * np.exp(1j * q * kx * alattice)
            h12T[q - 1, 0] = E_R4[2][1] + E_R4[2][1] * np.exp(-1j * q * kx * alattice)

            h22[0, q - 1] = E_R1[2][2] + E_R1[2][2] * np.exp(-1j * q * kx * alattice)
            h22[q - 1, 0] = E_R4[2][2] + E_R4[2][2] * np.exp(1j * q * kx * alattice)

        else:
            h0[0, q - 1] = E_R1[0][0] * np.exp(-1j * q * kx * alattice)
            h0[q - 1, 0] = E_R4[0][0] * np.exp(1j * q * kx * alattice)

            h1[0, q - 1] = E_R1[0][1] * np.exp(-1j * q * kx * alattice)
            h1[q - 1, 0] = E_R4[0][1] * np.exp(1j * q * kx * alattice)

            h2[0, q - 1] = E_R1[0][2] * np.exp(-1j * q * kx * alattice)
            h2[q - 1, 0] = E_R4[0][2] * np.exp(1j * q * kx * alattice)

            h1T[0, q - 1] = E_R1[1][0] * np.exp(1j * q * kx * alattice)
            h1T[q - 1, 0] = E_R4[1][0] * np.exp(-1j * q * kx * alattice)

            h11[0, q - 1] = E_R1[1][1] * np.exp(-1j * q * kx * alattice)
            h11[q - 1, 0] = E_R4[1][1] * np.exp(1j * q * kx * alattice)

            h12[0, q - 1] = E_R1[1][2] * np.exp(-1j * q * kx * alattice)
            h12[q - 1, 0] = E_R4[1][2] * np.exp(1j * q * kx * alattice)

            h2T[0, q - 1] = E_R1[2][0] * np.exp(1j * q * kx * alattice)
            h2T[q - 1, 0] = E_R4[2][0] * np.exp(-1j * q * kx * alattice)

            h12T[0, q - 1] = E_R1[2][1] * np.exp(1j * q * kx * alattice)
            h12T[q - 1, 0] = E_R4[2][1] * np.exp(-1j * q * kx * alattice)

            h22[0, q - 1] = E_R1[2][2] * np.exp(-1j * q * kx * alattice)
            h22[q - 1, 0] = E_R4[2][2] * np.exp(1j * q * kx * alattice)
    ...
    H2band[0:q, 0:q] = h11
    H2band[0:q, q : 2 * q] = h12
    H2band[q : 2 * q, 0:q] = h12T
    H2band[q : 2 * q, q : 2 * q] = h22
...
def main():
    ...
    qmax = 151  # int(input("Input the range q max aka the magnetic cell: "))
...
#set xrange [0:0.12]
#set yrange [1.3:2.0]
plot '{file}' u 1:2 with points pt 7 ps 0.3 lc rgb 'black' notitle 'HofstadterButterfly_{matt}'
pause -1
        """
        )
